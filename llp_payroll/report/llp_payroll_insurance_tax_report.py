...
class LLPPayrollInsuranceTaxReport(models.TransientModel):
    ...
    #         'border': 0,
    #     })
    #     file_name = 'Нийгмийн даатгал шимтгэлийн хураамжийн тайлан'
    ...
    def action_export(self):
        ...
        col_keys = [
            ...
            'job',
            'civil_number',
            'wages_and_equivalent_income',
            'insurance_tax_employer',
            'insurance_tax_employee',
            'insurance_type',
            'is_limit_max',
        ]
        ...
        for record in rows:
            for col_idx, key in enumerate(col_keys):
                ...
                if _is_number(value):
                    sheet.write_number(data_row, col_idx, float(value), num_fmt)
                else:
                    sheet.write(data_row, col_idx, value or '', cell_fmt)
            data_row += 1
